  ...
  constructor () {
    ...
    this.tap = {
      ...
      bind: (bindTarget, callback, delegateTarget) => {
        ...
        let lastClientX
        let lastClientY
        this.tap.eventHandler.set(callback, {
          touchstart: e => {
            e.preventDefault()
        ...
        bindTarget.addEventListener(
          'touchstart',
          this.tap.eventHandler.get(callback).touchstart
        )
        bindTarget.addEventListener(
          'touchend',
          this.tap.eventHandler.get(callback).touchend
        )
      },
      remove: (bindTarget, callback) => {
        return this._removeEvent('tap', bindTarget, callback)
      }
    }
    /**
     * longtap 自定义长按事件
