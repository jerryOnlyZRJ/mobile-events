          ...
          touchmove: e => {
            ...
            if (e.touches.length === 2) {
              if (!lastSpaceOf2Touch) {
                lastSpaceOf2Touch = getVectorLength(e.touches[0].clientX, e.touches[0].clientY, e.touches[1].clientX, e.touches[1].clientY)
              } else if (!timer.timer) {
                timer.timeoutCreator(50, () => {
                  if (!lastSpaceOf2Touch) return
                  const thisSpaceOf2Touch = getVectorLength(e.touches[0].clientX, e.touches[0].clientY, e.touches[1].clientX, e.touches[1].clientY)
            ...
            if (e.touches.length === 2) {
              if (!lastArrOf2Touch) {
                ...
                timer.timeoutCreator(50, () => {
                  ...
                  const lengthOfLast = getVectorLength(lastX1, lastY1, lastX2, lastY2)
                  const lengthOfThis = getVectorLength(thisX1, thisY1, thisX2, thisY2)
                  const rotateAngle =
                    (Math.acos(
