            ...
            if (e.touches.length === 2) {
              if (!lastSpaceOf2Touch) {
                ...
                timer.timeoutCreator(100, () => {
                  const thisSpaceOf2Touch = Math.sqrt(Math.pow(e.touches[0].clientX - e.touches[1].clientX, 2) + Math.pow(e.touches[0].clientY - e.touches[1].clientY, 2))
                  // matrix(0.75, 0, 0, 0.75, 0, 0)
                  const scale = Math.floor((thisSpaceOf2Touch * 100) / lastSpaceOf2Touch) / 100
                  e.scale = scale
                  callback(e)
                })
              }
            ...
            if (e.touches.length === 2) {
              if (!lastArrOf2Touch) {
                ...
                timer.timeoutCreator(100, () => {
                  ...
                  const lengthOfLast = Math.sqrt(Math.pow(lastX2 - lastX1, 2) + Math.pow(lastY2 - lastY1, 2))
                  const lengthOfThis = Math.sqrt(Math.pow(thisX2 - thisX1, 2) + Math.pow(thisY2 - thisY1, 2))
                  const rotateAngle = Math.acos((lastX2 - lastX1)(thisX2 - thisX1) + (lastY2 - lastY1)(thisY2 - thisY1) / (lengthOfLast * lengthOfThis)) * 90 / Math.PI
                  // 顺时针+， 逆时针-
                  const vectorCross = (lastX1 - thisX1) * (lastY2 - thisY2) - (lastX2 - thisX2) * (lastY1 - thisY1)
                  let rotate = {
                    rotateAngle
                  }
                  if (vectorCross < 0) {
